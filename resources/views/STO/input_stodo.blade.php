        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                {{-- <div class="row">
                    ...
                    <div class="col-md-12 mb-3">
                        <label for="" class="form-label">Select DO</label>
                        <select id="id_sto" class="form-select select2-add">
                            <option value="">-- Select DO --</option>
                            @foreach($do as $key => $value)
                                <option value="{{$value->id}}">{{$value->do_no}}</option>
                            @endforeach
                        </select>
                    </div>
                </div> --}}
                ...
                <div class="row mb-3">
                    <div class="col-lg-3 col-md-4 label">DO NO</div>
                    <div class="col-lg-9 col-md-8 fw-bold" data-name="do_no">-</div>
                </div>

                <div class="row mb-3">
                    <div class="col-lg-3 col-md-4 label">DO STATE</div>
                    <div class="col-lg-9 col-md-8 fw-bold" data-name="do_state">-</div>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
                <button type="button" class="btn btn-primary" data-name="save">Delivered</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).on("click", ".action-input", function (e) {
        ...
        var do_state    = $("[data-name='do_no']").text(': '+$(this).attr("data-item").split(",")[5]);
        var do_date     = $("[data-name='do_state']").text(': '+$(this).attr("data-item").split(",")[6]);
        $("#input").modal('show');
    });
    ...
    function fetchAndUpdateData(){
       $('#dataTable').DataTable({
           ...
           rowCallback: function (row, data) {
            ...
            var attributeValue  = data.id+','+data.so_no+','+data.pt_no+','+data.wo_state+','+data.wo_date+','+data.do_no+','+data.do_state;
