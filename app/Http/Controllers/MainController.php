    ...
    function import(Request $request){
        ...
        foreach ($sheetData as $key => $row) {
            ...
            if (strpos($row[3], '/') !== false) {
                $wo_date_exc    = str_replace('/', '-', $row[3]);
            } else {
                $wo_date_exc    = $row[3];
            }
            $wo_date        = date("Y-m-d H:i:s", strtotime($wo_date_exc));
            // $wo_date        = $row[3];
            $do_no          = $row[4];
            if (strpos($row[5], '/') !== false) {
                $do_date_exc    = str_replace('/', '-', $row[5]);
            } else {
                $do_date_exc    = $row[5];
            }
            $do_date        = date("Y-m-d H:i:s", strtotime($do_date_exc));
            // $do_date        = $row[5];
            $do_state       = $row[6];
